...
def binomial(idx, successes, populationTotal):
    p = math.factorial(P) / (math.factorial(i) * math.factorial(P-i)) * (0.25**i) * (0.75**(P-i))
    return p
...
ans = '{:.3f}'.format(sum([binomial(i, N, P) for i in range(N, P+1)])) # need to round off to 3 dec
